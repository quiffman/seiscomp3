 ...
 * @return basename */
SC_CORE_UTILS_API std::string basename(const std::string& name);

//! Checks if a file is exists
SC_CORE_UTILS_API bool fileExists(const std::string& file);

//! Checks if a path (directory) exists
SC_CORE_UTILS_API bool pathExists(const std::string& path);

//! Creates a new directory inclusive all unavailable
//! parent directories
SC_CORE_UTILS_API bool createPath(const std::string& path);

/** Removes the extension from the filename
 * test.xyz -> test
 * @return name without extension */
SC_CORE_UTILS_API std::string removeExtension(const std::string& name);
 ...
 */
SC_CORE_UTILS_API std::streambuf *bytesToStreambuf(char *data, size_t n);
SC_CORE_UTILS_API std::streambuf *stringToStreambuf(const std::string &str);
