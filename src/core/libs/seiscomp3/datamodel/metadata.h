...
#ifndef __SEISCOMP_DATAMODEL_METADATA_H__
#define __SEISCOMP_DATAMODEL_METADATA_H__
...
class EnumPropertyBase<T, U, F1, F2, 0> : public Core::MetaProperty {
	public:
		...
		std::string readString(Core::BaseObject *object) const throw(Core::GeneralException) {
			const T *target = T::ConstCast(object);
			if ( !target ) throw Core::GeneralException("invalid object");
