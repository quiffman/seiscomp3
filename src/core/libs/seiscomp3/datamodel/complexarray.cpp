...
IMPLEMENT_METAOBJECT(ComplexArray)
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void ComplexArray::setContent(const std::vector< std::complex<double> > &content) {
	_content = content;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
void ComplexArray::serialize(Archive& ar) {
	...
	if ( ar.isHigherVersion<0,6>() ) {
		SEISCOMP_ERROR("Archive version %d.%d too high: ComplexArray skipped",
