...
{
	const char* installDir = getenv("SEISCOMP3_ROOT");
	if (installDir == NULL)
	{
#ifdef SEISCOMP3_ROOT
		_installDir = SEISCOMP3_ROOT;
	...
	else
	{
		...
		SEISCOMP_DEBUG("Setting installdir from $SEISCOMP3_ROOT: %s", _installDir.c_str());
	}


#ifdef SEISCOMP3_SHARE_DIR
	_shareDir = _installDir + "/"SEISCOMP3_SHARE_DIR;
#else
...
#else
	_globalConfigDir  = _installDir + "/etc";
#endif

	_appConfigDir = _installDir + "/config";

	const char* localConfigDir = getenv("SEISCOMP3_LOCAL_CONFIG");
...
bool Environment::initConfig(Config* config, const std::string& name,
                             int fromStage, int toStage) const {
