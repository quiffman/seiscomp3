	...
	for ( NameList::const_iterator it = _pluginNames.begin();
		...
		if ( e.plugin == NULL ) {
			if ( e.handle == NULL )
				...
				SEISCOMP_WARNING("The plugin %s has been loaded already", it->c_str());
			continue;
		}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
int PluginRegistry::loadConfiguredPlugins(const Config *config) {
...
PluginRegistry::PluginEntry PluginRegistry::open(const std::string &file) const {
	...
	// Load factory
	Core::Plugin::CreateFunc func = (Core::Plugin::CreateFunc)dlsym(handle, "createSCPlugin");
#else
