...
#ifdef DLSV2INV
DataModel::Inventory* g_inventory;
#endif
...
Inventory::Inventory(INIT_MAP& init,
	...
	init_values(init), _connection(connection)
{
...
#ifdef DLSV2INV
		g_inventory = new DataModel::Inventory();
...
#ifdef DLSV2INV
		IO::XMLArchive ar;
		if(ar.create((net_code + "." + sta_code + "." + date2str(GetTime(sta_start)) + ".xml").c_str()))
			ar << g_inventory;
		else
			SEISCOMP_ERROR("cannot open %s.%s.%s.xml", net_code.c_str(), sta_code.c_str(),
				date2str(GetTime(sta_start)).c_str());
#else
		SendNotifiers();
#endif
	}
	...
	for(unsigned int i=0; i<si.ci.size(); i++)
	{
		...
		if(IsPAZStream(ci) || IsPolyStream(ci))
		{
			...
			*/
			...
			if(!strm)
				strm = InsertStream(ci, loc);
			else
				UpdateStream(ci, strm); //, ins.second);
		...
		else
		{
			...
			if(!strm)
				strm = InsertAuxStream(ci, loc);
			else
				UpdateAuxStream(ci, strm);
		}
	}
}
...
* Description:	insert a new channel(seisstream)									    *
****************************************************************************************************************************/
DataModel::StreamPtr Inventory::InsertStream(ChannelIdentifier& ci, DataModel::SensorLocationPtr loc)
{
...
* Description:	update the current channel with information from dataless						    *
****************************************************************************************************************************/
void Inventory::UpdateStream(ChannelIdentifier& ci, DataModel::StreamPtr strm) //, bool FirstComponent)
{
	...
	// Arbitrary values not allowed here, must be "Steim1" or "Steim2" currently.
	strm->setFormat("Steim2");
...
****************************************************************************************************************************/
DataModel::AuxStreamPtr Inventory::InsertAuxStream(ChannelIdentifier& ci, DataModel::SensorLocationPtr loc)
{
