...
class SDS(object):
    ...
    def __is_sds(self, time1, time2, net, sta, cha, loc, dir):
        """
        ...
        if os.path.exists(spath):
            for doy in [fname.split(".")[-1] for fname in os.listdir(spath)]:
                if doy >= sdoy and (spath != epath or doy <= edoy):
                    return True
        
        if os.path.exists(epath):
            for doy in [fname.split(".")[-1] for fname in os.listdir(epath)]:
        ...
        for i in range(1, time2.year-time1.year):
            ...
            sdsfile = os.path.join(dir, self.__get_sds_name(t,net,sta,cha,loc))
            if os.path.exists(os.path.dirname(sdsfile)) and os.listdir(os.path.dirname(sdsfile)):
                return True
            
        return False
        

    def __is_iso(self, time1, time2, net, sta):
        """
    ...
    def exists(self, time1, time2, net, sta, cha, loc):
        return self.__is_iso(time1, time2, net, sta) or \
            self.__is_sds(time1, time2, net, sta, cha, loc, self.archdir) or \
            self.__is_sds(time1, time2, net, sta, cha, loc, self.nrtdir)
    ...
    def iterdata(self, time1, time2, net, sta, cha, loc):
        ...
        if self.__is_iso(time1, time2, net, sta):
