...
RecordSequence::findInsertPosition(const Record *rec, iterator *it)
{
	...
	if ( rec->startTime() < front()->startTime() ) {
		if ( it ) *it = begin();
		return true;
	}
	...
	reverse_iterator next = current++;

	for ( ; current != rend(); next = current++ ) {
		if ( rec->endTime() > (*next)->startTime() ) return false;
		if ( rec->startTime() < (*current)->endTime() ) continue;
