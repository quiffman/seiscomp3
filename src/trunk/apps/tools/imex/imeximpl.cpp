...
{
	try {
		_filter = _imex->configGetBool(_sinkName + ".filter");
	}
	...
	try {
		_useDefinedRoutingTable = _imex->configGetBool(_sinkName + ".useDefinedRoutingTable");
	}
	catch ( ConfigException& e ) {
	...
	try {
		_conversion = _imex->configGetString(_sinkName + ".conversion");
	...
	// Get sink address
	try {
		_sinkAddress = _imex->configGetString(_sinkName + ".address");
	}
	catch ( ConfigException& e ) {
	...
	try {
		_userName = _imex->configGetString(_sinkName + ".userName");
	}
	catch ( ConfigException& ) {}
	...
	try {
		criteriaStr = _imex->configGetString(_sinkName + ".criteria");
	}
	catch ( ConfigException& e ) {
		SEISCOMP_DEBUG("(%s) %s ", e.what(), _sinkName.c_str());
		return false;
...
bool ImExImpl::buildRoutingTable()
{
	...
	if ( _useDefinedRoutingTable ) {
		if (!configGetRoutingTable(_sinkName, "routingTable", &_routingTable))
			return false;
	}
