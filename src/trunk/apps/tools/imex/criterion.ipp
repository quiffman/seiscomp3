...
T* CriterionFactory<T>::createExpression(const std::string& name)
{
	...
	if (!configGetLatitude(name, "latitude", criterion))
	{
		delete criterion;
		return NULL;
	}

	if (!configGetLongitude(name, "longitude", criterion))
	{
		delete criterion;
		return NULL;
	}

	if (!configGetMagnitude(name, "magnitude", criterion))
	{
		delete criterion;
		return NULL;
	}

	if (!configGetArrivalCount(name, "arrivalCount", criterion))
	{
		delete criterion;
		return NULL;
	}

	if (!configGetAgencyID(name, "agencyid", criterion))
	{
		delete criterion;
		return NULL;
	}
