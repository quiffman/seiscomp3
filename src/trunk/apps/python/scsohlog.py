...
class ObjectLog:
  ...
  def toXML(self, f, name, channel):
    f.write('<object')
    if name: f.write(' name="%s"' % name)
    if channel: f.write(' channel="%s"' % channel)
    if not self.count is None: f.write(' count="%s"' % self.count)
    if not self.timeWindow is None: f.write(' timeWindow="%s"' % self.timeWindow)
    if not self.average is None: f.write(' average="%s"' % self.average)
    if self.last: f.write(' lastTime="%s"' % self.last)
...
class Client:
  ...
  def updateTests(self, updateTime, tests):
    for name,value in tests.items():
      if name == "pid": self.pid = value
      elif name == "programname": self.progname = value
      elif name == "hostname": self.host = value

      if not Tests.has_key(name): continue

      # Convert d:h:m:s to seconds
      if name == "uptime":
        try: t = [int(v) for v in value.split(":")]
        except: continue
        if len(t) != 4: continue
  ...
  def updateObjects(self, updateTime, log):
    ...
    try: tmp = v.group('params').split(',')
    except: return

    params = dict()
    for p in tmp:
      try: param,value = p.split(':',1)
      except: continue
      params[param] = value
  ...
  def toXML(self, f, name):
    ...
    if self.host: f.write(' host="%s"' % self.host)
    if self.pid: f.write(' pid="%s"' % self.pid)
    if self.progname: f.write(' prog="%s"' % self.progname)
    f.write('>')
    for name,log in self.tests.items(): log.toXML(f, name)
    if len(self.inputLogs) > 0:
      f.write('<input>')
      for id,log in self.inputLogs.items(): log.toXML(f, id[0], id[1])
      f.write('</input>')
    if len(self.outputLogs) > 0:
      f.write('<output>')
      for id,log in self.outputLogs.items(): log.toXML(f, id[0], id[1])
      f.write('</output>')
    f.write("</service>")
...
class Monitor(seiscomp3.Client.Application):
  def __init__(self, argc, argv):
    ...
    self.addMessagingSubscription(seiscomp3.Communication.Protocol.STATUS_GROUP)
  ...
  def createCommandLineDescription(self):
    try:
      ...
      self.commandline().addStringOption("Output", "file,o", "Specify the output file to create")
      self.commandline().addIntOption("Output", "interval,i", "Specify the output interval in seconds (default: 60)")
      self.commandline().addStringOption("Output", "script", "Specify an output script to be called after the output file is generated")
    except:
  ...
  def initConfiguration(self):
    ...
    try: self._outputFile = self.configGetString("monitor.output.file")
    except: pass

    try: self._outputInterval = self.configGetInt("monitor.output.interval")
    except: pass

    try: self._outputScript = self.configGetString("monitor.output.script")
    except: pass
  ...
  def init(self):
    ...
    try: self._outputFile = self.commandline().optionString("file")
    except: pass

    try: self._outputInterval = self.commandline().optionInt("interval")
    except: pass
    ...
    seiscomp3.Logging.info("Starting output timer with %d secs" % self._outputInterval)
  ...
  def handleTimeout(self):
    ...
    try: f = open(self._outputFile, "w")
    except:
      seiscomp3.Logging.error("Unable to create output file: %s" % self._outputFile)
      return
