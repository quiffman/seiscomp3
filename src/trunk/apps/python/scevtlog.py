...
class EventHistory(seiscomp3.Client.Application):
  def __init__(self, argc, argv):
    ...
    self._cache = seiscomp3.DataModel.PublicObjectTimeSpanBuffer(self.query(), seiscomp3.Core.TimeSpan(30.0*60.0))
  ...
  def init(self):
    ...
    try: self._format = self.commandline().optionString("format")
    except: pass
  ...
  def printEventProcAlert(self, evt, newEvent):
    ...
    if org:
      ...
      for i in xrange(narr):
        picks.append(self._cache.get(seiscomp3.DataModel.Pick, org.arrival(i).pickID()))

      nstamags = org.stationMagnitudeCount()
      for i in xrange(nstamags):
        amps.append(self._cache.get(seiscomp3.DataModel.Amplitude, org.stationMagnitude(i).amplitudeID()))
    ...
    if self._directory is None:
      ...
      try: arNow = evt.creationInfo().creationTime().get()
      ...
      if directory != self._currentDirectory:
        if createDirectory(directory) == False:
          seiscomp3.Logging.error("Unable to create directory %s" % directory)
          return
  ...
  def printEventXML(self, evt, newEvent):
    ...
    if org:
      ...
      for i in xrange(ncmts):
        cmt_cloned = seiscomp3.DataModel.Comment.Cast(evt.comment(i).clone())
        evt_cloned.add(cmt_cloned)

      # Copy origin references
      org_ref = evt.originReference(seiscomp3.DataModel.OriginReferenceIndex(org.publicID()))
      if org_ref:
        org_ref_cloned = seiscomp3.DataModel.OriginReference.Cast(org_ref.clone())
        if org_ref_cloned is None: org_ref_cloned = seiscomp3.DataModel.OriginReference(org.publicID())
        evt_cloned.add(org_ref_cloned)

      # Copy event descriptions
      for i in xrange(evt.eventDescriptionCount()):
        ed_cloned = seiscomp3.DataModel.EventDescription.Cast(evt.eventDescription(i).clone())
      ...
      for i in xrange(ncmts):
        cmt_cloned = seiscomp3.DataModel.Comment.Cast(org.comment(i).clone())
      ...
      for i in xrange(nmag):
        ...
        for j in xrange(nmagref):
          mag_ref_cloned = seiscomp3.DataModel.StationMagnitudeContribution.Cast(mag.stationMagnitudeContribution(j).clone())
      ...
      for i in xrange(smag):
        mag_cloned = seiscomp3.DataModel.StationMagnitude.Cast(org.stationMagnitude(i).clone())
        org_cloned.add(mag_cloned)
  ...
  def objectAboutToPop(self, obj):
    try:
      ...
      if org:
        try: self._orgToEvent.pop(org.publicID())
        except: pass
        return

      mag = seiscomp3.DataModel.Magnitude.Cast(obj)
      if mag:
        try: self._magToEvent.pop(mag.publicID())
        except: pass
        return
    except:
  ...
  def eventProgress(self, evtID, directory):
    ...
    for file in files:
      if os.path.isfile(directory + file) == False: continue
      fid = file[len(evtID + '.'):len(file)]
      sep = fid.find('.')
      if sep == -1: sep = len(fid)
      fid = fid[0:sep]
      try: nid = int(fid)
      except: continue
