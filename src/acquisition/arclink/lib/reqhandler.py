...
class WiggleFetcher(object):
    def __init__(self, nrtdir, archdir, isodir, max_size, dcid, subnode_addr,
        dcid_override):
        self.__volume_factory = None
        self.__local_volume_fd = None
        self.__local_volume_size = 0
        self.__request_size = 0
        self.__sds = sds.SDS(nrtdir, archdir, isodir)
...
class RequestHandlerApp(Client.Application):
    ...
    def initConfiguration(self):
        ...
        try:
            self.reqdir = self.configGetString("reqdir")
        ...
        try:
            self.trackdb = self.configGetString("trackdb")
        ...
        try:
            self.trackdir = self.configGetString("trackdir")

        except Config.ConfigException:
            self.trackdir = None

        try:
            self.nrtdir = self.configGetString("nrtdir")

        except Config.ConfigException:
            logs.error("NRT SDS directory not specified")
            return False

        try:
            self.archdir = self.configGetString("archdir")

        except Config.ConfigException:
            logs.error("SDS archive directory not specified")
            return False

        try:
            self.isodir = self.configGetString("isodir")

        except Config.ConfigException:
            logs.error("ISO archive directory not specified")
            return False

        try:
            self.gfaurl = self.configGetString("gfaurl")

        except Config.ConfigException:
            self.gfaurl = None

        try:
            self.subnodelist = self.configGetString("subnodelist")
        ...
        try:
            self.maxsize = self.configGetInt("maxsize")
            ...
            logs.error("maximum request size not specified")
            return False
    ...
    def run(self):
        try:
            ...
            wf = WiggleFetcher(self.nrtdir, self.archdir, self.isodir,
...
if __name__ == "__main__":
    ...
    app = RequestHandlerApp(len(sys.argv), sys.argv)
    sys.exit(app())
